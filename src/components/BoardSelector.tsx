        ...
        <div className="h-full overflow-y-auto pb-20">
          <div className="sticky top-0 z-20 bg-background p-4 border-b">
            <div className="flex items-center justify-between">
              ...
              <h2 className="text-xl font-bold text-brand-600">Let's Start</h2>
              <div className="w-8"></div> {/* Spacer for alignment */}
            </div>
          </div>

          <div className="p-4">
            <div className="bg-brand-600 text-white py-2 px-4 rounded-lg text-center mb-6">
              Board/Exam
            </div>
            ...
            {boards.map((board) => (
              <motion.div
                ...
                className={`border border-brand-200 rounded-full p-4 mb-4 flex items-center justify-between ${
                  selectedBoards.includes(board.id) ? 'border-brand-600' : ''
                }`}
            ...
            <div className="bg-brand-600 text-white py-2 px-4 rounded-lg text-center my-6">
              Paper Medium
            </div>
            ...
            {languages.map((lang) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className={`border border-brand-200 rounded-full p-4 mb-4 flex items-center justify-between ${
                  selectedLanguage === lang.id ? 'border-brand-600' : ''
                }`}
